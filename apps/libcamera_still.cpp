...
public:
	...
	int countdown_;
	std::chrono::time_point<std::chrono::high_resolution_clock> ttt_;
...
static void event_loop(LibcameraStillApp &app)
{
	...
	XGrabKey(dpy, XKeysymToKeycode(dpy, XK_space), 0, root, 0, GrabModeAsync, GrabModeAsync);
	...
	for (unsigned int count = 0;; count++)
	{
		...
		while(XPending(dpy)) {
			...
			if (ev.type == KeyPress) {
				app.countdown_ = 3;
				//keypressed = true;
				app.ShowCountdown(app.countdown_, 1.2f);
		...
		if (app.countdown_ > 0) {
			...
			if (now - app.ttt_ >= std::chrono::seconds(1)) {
				app.countdown_--;
				if (app.countdown_ == 0) {
					app.ShowCountdown(app.countdown_, 0.5f);
					keypressed = true;
				} else {	
					app.ttt_ = now;
					app.ShowCountdown(app.countdown_, 1.2f);
